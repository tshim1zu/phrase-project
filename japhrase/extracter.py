...
class PhraseExtracter:
    ...
    チューニング:
        >>> pe = PhraseExtractor()
        >>> pe.tune(texts)              # テキストに合わせてパラメータ最適化
        >>> pe.show_params()            # パラメータ確認（コピペ用コード付き）
        >>> pe.save_params("p.json")    # 保存（明示的）
        >>> pe = PhraseExtractor.load_params("p.json")  # 復元（明示的）
    """
    ...
    def show_params(self):
        ...
        if tuned:
            print()
            print("  保存: pe.save_params('params.json')")
            print("  復元: pe = PhraseExtractor.load_params('params.json')")
        print("=" * 55)
    ...
    def _tune_optuna(self, texts, n_trials, verbose):
        ...
        def objective(trial):
            ...
            """
            # 抽出閾値: 範囲を絞って ranking パラメータが見えるようにする
            min_count = trial.suggest_int(  'min_count',             2, min_count_max)
            max_length = trial.suggest_int( 'max_length',            6, 16)   # 実用範囲に限定
            min_length = trial.suggest_int( 'min_length',            3,  4)   # 3か4の2択
            ...
            # PMI: 日本語（文字N-gram）で効く。英語（単語N-gram）は固定
